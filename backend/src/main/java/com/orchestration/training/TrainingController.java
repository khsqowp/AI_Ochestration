...
public class TrainingController {
  ...
  @GetMapping("/black-box/sessions") public List<BlackBoxSessionResponse> blackBoxHistory(@CookieValue(value=COOKIE,required=false)String token,@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size){return blackBox().history(owner(token),page,size).stream().map(BlackBoxSessionResponse::from).toList();}
  ...
  // 레이더/추이 차트 전용 -- 세션 본문(messages/observations) 없이 skillCode+score+closedAt만.
  @GetMapping("/black-box/sessions/scores") public List<ScorePointResponse> blackBoxScoreTrend(@CookieValue(value=COOKIE,required=false)String token){return blackBox().scoreTrend(owner(token)).stream().map(ScorePointResponse::from).toList();}
