#!/usr/bin/env python3
"""Marker-based Reflected-XSS scanner (single page only, non-DOM).

Implements the funnel workflow instead of spraying live payloads at every
parameter: inject a unique, harmless marker per parameter first, find where
it reflects IN THE SAME RESPONSE, classify the surrounding context (HTML
text / attribute / JavaScript string / URL attribute / JSON) and which
probe characters survived unescaped there, and only then flag genuinely
dangerous candidates. Real payload execution / exploitation is never
attempted -- final confirmation of any HIGH candidate is a manual/Burp
step.

Scope: exactly ONE target URL, checked against its OWN response only.
This tool never crawls or follows links to other pages -- for that kind of
cross-page checking see the sibling xss_stored_scanner.py, which itself is
also capped to a single fixed check page (never an open-ended crawl), to
keep total request volume small and predictable.

Marker shape: "<param>_XSS_TEST_<random6hex>" immediately followed by the
raw probe charset  <>'"/`  -- e.g. for a param named jsessionid:
jsessionid_XSS_TEST_a1b2c3<>'"/`
The "core" (name+XSS_TEST+random) is always sent as literal, searchable
plain text; only the trailing probe charset is optionally transformed by
--bypass-variants (HTML-entity decimal/hex/named, double URL-encoding,
base64) to test for filter/WAF bypass or double-decode bugs -- detection
always just searches the response for the literal core string, regardless
of which encoding was used to send it, then inspects what follows it.

Explicitly OUT OF SCOPE: DOM-based XSS (client-side sinks like innerHTML,
document.write, location.hash). That requires a headless browser
instrumenting JS sinks, not response-text marker search.

Usage:
    python xss_reflected_scanner.py https://target.example/search --params q,category,sort
    python xss_reflected_scanner.py https://target.example/login --method post-form --params username,memo
    python xss_reflected_scanner.py https://target.example/api/update --method post-json --params name,memo
    python xss_reflected_scanner.py https://target.example/ --cookie-params nickname,lang
    python xss_reflected_scanner.py https://target.example/ --header-params X-Forwarded-For
    python xss_reflected_scanner.py https://target.example/search --params q --bypass-variants all
"""
...
logger = logging.getLogger("xss_reflected_scanner")

DEFAULT_USER_AGENT = "xss-reflected-scanner/1.0 (+rate-limited; contact: local-security-testing)"
...
# ---------------------------------------------------------------------------
# Context classification + probe-survival
# ---------------------------------------------------------------------------
...
def probe_survival(after: str) -> tuple[bool, str]:
    ...
    (immediately adjacent to the marker core -- that's where the probe was
    sent). For each char: raw match = survived unescaped (dangerous); a
    known HTML-entity form = escaped (safe-ish); anything else stops the
    walk. This never scans an arbitrary window, so it can't misattribute
    unrelated surrounding markup (e.g. a literal '<' from '</div>') or a
    different nearby marker occurrence to this one."""
    pos = 0
    survived: list[str] = []
...
def build_and_fetch(
    ...
    """Returns (response_text, content_type, request_url_for_report, status, error)."""
...
# Reporting
# ---------------------------------------------------------------------------
def print_console_report(hits: list[ReflectionHit], total_requests: int) -> None:
    print("=" * 60)
    print(" Reflected-XSS Marker Scan Report (single page)")
    print("=" * 60)
    ...
    print(f"\nTotal requests sent: {total_requests}  |  Reflections found: {len(hits)}  (candidates need manual/Burp confirmation for final verdict)")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="xss_reflected_scanner", description="Marker-based Reflected-XSS scanner (single page only, non-DOM).")
    parser.add_argument("url", help="Target URL (checked against its own response only -- no crawling)")
    parser.add_argument("--method", choices=["get", "post-form", "post-json"], default="get", help="HTTP method/body encoding (default: get)")
    ...
    parser.add_argument("--cookies", default=None, help="Baseline Cookie header sent with every request, 'k=v; k2=v2' style (e.g. session auth)")
    parser.add_argument("--headers", action="append", default=[], help="Baseline header 'Name: value', repeatable")
    ...
    parser.add_argument("--batch-size", type=int, default=10, help="Params combined per request (default 10; >20 needs --force -- server logic can interact across params)")
    parser.add_argument("--workers", type=int, default=3, help="Reserved for parity with sibling tools (requests here are sequential/rate-limited, not parallel)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("Injecting %d marker slot(s) against a single URL in batches of %d, variants=%s", len(slots), args.batch_size, ",".join(variants))
    for batch in batch_slots(slots, args.batch_size):
        rate_limiter.wait(host_key)
        text, content_type, req_url, status, error = build_and_fetch(
        ...
        hits = find_reflections(text, content_type, cores)
        all_hits.extend(hits)
        logger.info("Batch of %d slot(s) -> status=%s, %d reflection(s)", len(batch), status, len(hits))
    ...
    if args.output == "json":
        ...
        print_console_report(all_hits, total_requests)
        if args.output_file:
            write_json_report(all_hits, args.output_file)
