        ...
        <table className="order-grid">
          ...
          <tbody>
            {rows.map((r, i) => <tr key={r.id ?? i}>
              ...
              <td><input className="order-qty" type="number" min={1} max={999} value={r.altQty} onChange={e => patch(i, { altQty: Number(e.target.value) })} disabled={!r.altProduct.trim()}/></td>
              <td><select value={r.altUnit} onChange={e => patch(i, { altUnit: e.target.value as OrderUnit })} disabled={!r.altProduct.trim()}>{UNITS.map(u => <option key={u} value={u}>{UNIT_LABEL[u]}</option>)}</select></td>
              <td><button className="order-row-del" onClick={() => setRows(rs => rs.filter((_, idx) => idx !== i))} aria-label="행 삭제"><Trash2 size={15}/></button></td>
            </tr>)}
          </tbody>
        </table>
      </div>
