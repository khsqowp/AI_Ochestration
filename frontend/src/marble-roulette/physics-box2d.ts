...
export class Box2dPhysics implements IPhysics {
  ...
  createMarble(id: number, x: number, y: number): void {
    ...
    const body = this.world.CreateBody(bodyDef);
    // 밀도 지터 = 결과(구슬끼리 충돌 시 밀리는 정도)에 영향을 주는 랜덤이라 시드된 gameRandom을
    // 쓴다(원본은 Math.random)
    body.CreateFixture(circleShape, 1 + gameRandom.next());
    body.SetAwake(false);
  ...
  shakeMarble(id: number): void {
    ...
    if (body) {
      // 정지 탈출 충격 방향/세기 = 결과에 영향을 주는 랜덤이라 시드된 gameRandom을 쓴다(원본은 Math.random)
      body.ApplyLinearImpulseToCenter(
        new this.Box2D.b2Vec2(gameRandom.next() * 10 - 5, gameRandom.next() * 10 - 5),
        true
      );
    }
  }
