...
/** 원본(lazygyu/roulette)에는 광고 오버레이/광고판 렌더링과 스트리머 채널 전용 이름→이미지
 * 하드코딩 목록, 유료 스킨 스프라이트 서비스(KeywordService) 연동이 있었다 -- 사내 도구라
 * 전부 뺐다. 이름이 매칭되는 스킨이 없으면 원본도 색칠된 원으로 그리므로(marble.ts의 skin
 * undefined 분기) 동작은 그대로 남는다. */
export class RouletteRenderer {
  ...
  private _resultCloseRect: { x: number; y: number; w: number; h: number } | null = null;
  ...
  getResultCloseHitAt(x: number, y: number): boolean {
    const rect = this._resultCloseRect;
  ...
  private renderWinnerList(winners: Marble[], { theme, winnerRange }: RenderParameters) {
    ...
    const closeR = Math.max(12, Math.min(24, h * 0.03));
    this._resultCloseRect = {
      x: panelX + panelW - closeR,
      y: panelY - closeR,
      w: closeR * 2,
      h: closeR * 2,
    };
    ctx.beginPath();
    ctx.fillStyle = '#222';
    ctx.arc(panelX + panelW, panelY, closeR, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = 'white';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(panelX + panelW - closeR * 0.4, panelY - closeR * 0.4);
    ctx.lineTo(panelX + panelW + closeR * 0.4, panelY + closeR * 0.4);
    ctx.moveTo(panelX + panelW + closeR * 0.4, panelY - closeR * 0.4);
    ctx.lineTo(panelX + panelW - closeR * 0.4, panelY + closeR * 0.4);
    ctx.stroke();
  ...
  private renderWinner(winner: Marble, theme: ColorTheme) {
    ...
    // Draw colored circle (원본은 이름 매칭 스킨 이미지가 있으면 그 위에 그렸다 -- 스킨 없음)
