...
/** lazygyu/roulette(MIT) 엔진 포팅 -- 광고 시스템(AdService/adRenderer)과 분석 트래킹(umami),
 * 유료 스킨 스프라이트 연동(KeywordService)은 사내 도구라 제외했다. 물리/렌더링/카메라/미니맵/
 * 스킬 이펙트/파티클/랭킹/배속/녹화/멀티맵은 그대로 가져왔다. */
export class Roulette extends EventTarget {
  ...
  // 화살표 필드로 바인딩한다 -- window.requestAnimationFrame(this._update)처럼 메서드를 그냥
  // 뜯어서 넘기므로(this 유실) 일반 메서드로 두면 안 된다. 원본은 레거시 데코레이터(@bound)로
  // 같은 효과를 냈는데, 우리 tsconfig는 experimentalDecorators가 꺼져있어 그 문법이 컴파일
  // 에러가 난다.
  private _update = () => {
    if (this._destroyed) return;
    ...
    this._render();
    this._rafHandle = window.requestAnimationFrame(this._update);
  };
  ...
  // 호출부(pointerdown 리스너, MouseMove/DblClick의 .bind(this, ev))가 항상 this를 명시적으로
  // 넘기므로 바인딩 불필요.
  ...
  /** React 언마운트 시 호출 -- rAF 루프를 멈추고 캔버스를 떼어낸다. 원본(vanilla 단일 페이지 앱)엔
   * 없던 메서드다: 원본은 페이지가 곧 인스턴스 생명주기라 정리할 일이 없었지만, 우리는 SPA
   * 라우트를 드나들 때마다 마운트/언마운트되므로 안 끊으면 rAF와 physics world가 누수된다. */
  public destroy() {
