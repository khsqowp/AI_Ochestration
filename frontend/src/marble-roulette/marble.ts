...
export class Marble {
  ...
  constructor(physics: IPhysics, order: number, max: number, name?: string, weight: number = 1) {
    ...
    // 결과(누가 먼저 스킬을 쓰는지)에 영향을 주는 랜덤이라 시드된 gameRandom을 쓴다(원본은 Math.random)
    this._coolTime = this._maxCoolTime * gameRandom.next();
  ...
  private _updateSkillInformation(deltaTime: number) {
    ...
    if (this._coolTime <= 0) {
      // 스킬 발동 여부 = 결과에 영향을 주는 랜덤이라 시드된 gameRandom을 쓴다(원본은 Math.random)
      this.skill = gameRandom.next() < this._skillRate ? Skills.Impact : Skills.None;
