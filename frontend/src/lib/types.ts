...
export type MomentumRotationPosition = { side: 'long' | 'short'; entryPrice: number; notionalUsdt: number; unrealizedPnlUsdt: number }
export type MomentumBroker = { queriedTs: string | null; exchange: string; equityUsdt: number; inceptionEquityUsdt: number; grossNotionalUsdt: number; returnPct: number; unrealizedPnlUsdt: number; drawdown: number; hwmUsdt: number; leverage: number; halted: boolean; positions: Record<string, MomentumRotationPosition> }
...
export type RotationBroker = { queriedTs: string | null; positions: Record<string, RotationBrokerPosition>; positionsEval: number; positionsEntry: number; positionsUnrealizedPnl: number; accountCashKrw: number; accountTotalKrw: number }
