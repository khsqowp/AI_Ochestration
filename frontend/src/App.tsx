...
function TrxTradingDashboard({ onClose, embedded }: { onClose: () => void; embedded?: boolean }) {
  ...
  return <Wrap embedded={embedded} onClose={onClose} eyebrow="TRADER Q" title="TRX 스윙 대시보드">
    ...
    <div className="trading-status-card">
      <BotStatusPill tone="live" label={data?.position ? '가동 중 · 보유 중' : '가동 중 · 골든크로스 대기'}/>
      <p>누적 실현손익 {data ? <b className={netPnl >= 0 ? 'positive' : 'negative'}>{netPnl >= 0 ? '+' : ''}${netPnl.toFixed(2)}</b> : '-'} (실현 {data ? (data.cumulativeRealizedPnlUsdt ?? 0).toFixed(2) : '-'} - 수수료 {data ? (data.cumulativeFeeUsdt ?? 0).toFixed(2) : '-'})</p>
      <p>시작일: {data?.inceptionTs ? fmtDate(data.inceptionTs) : '아직 시작 전'}</p>
    </div>
    {data ? <>
      <PeriodTabs period={period} onChange={setPeriod}/>
