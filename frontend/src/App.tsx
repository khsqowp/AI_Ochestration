import { Component, lazy, Suspense, useEffect, useMemo, useRef, useState, type CSSProperties, type FormEvent, type KeyboardEvent as ReactKeyboardEvent } from 'react'
...
import { DolphinChatModal } from './DolphinChatModal'
...
type Panel = 'sources' | 'archive' | 'agents' | 'usage' | 'digest' | 'ask' | 'settings' | 'help' | 'timeline' | 'files' | 'graph' | 'processes' | 'trading' | 'kr-trading' | 'us-trading' | 'momentum-rotation-trading' | 'calendar' | 'cheatsheet' | 'users' | 'debate' | 'dolphin' | null
...
class ErrorBoundary extends Component<{ label: string; children: React.ReactNode }, { failed: boolean }> {
  state = { failed: false }
...
type BotTone = 'live' | 'deprecated' | 'paused'
function BotStatusPill({ tone, label }: { tone: BotTone; label: string }) {
...
type RealTradingTab = 'trading' | 'momentum-rotation-trading'
