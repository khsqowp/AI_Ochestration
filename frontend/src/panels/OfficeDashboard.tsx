...
export function OfficeDashboard({ recentTasks, taskTracks, archivedCount, pendingCandidates, budgetExceeded, isAdmin, chatInput, setChatInput, taskDomain, setTaskDomain, chatError, onSubmitTask, onCancelTask, onOpenFile, todos }: {
  ...
  return <div className="office-dashboard">
    <DashboardSidebarChat isAdmin={isAdmin} recentTasks={recentTasks} taskTracks={taskTracks} chatInput={chatInput} setChatInput={setChatInput} taskDomain={taskDomain} setTaskDomain={setTaskDomain} chatError={chatError} onSubmitTask={onSubmitTask} onOpenFile={onOpenFile}/>
    <div className="dashboard-main">
      <div className="dashboard-grid">
        ...
        {isAdmin && <button className="dashboard-tile" onClick={() => scrollTo('dash-sources')}><span className="dashboard-tile-icon"><Globe2 size={19}/></span><b>{pendingCandidates}</b><span>수집 후보 대기</span></button>}
        ...
        {isAdmin && (() => { const r = momentumTrading?.broker?.sessionReturnPct ?? 0; return <button className={`dashboard-tile ${momentumTrading?.halted ? 'tone-deprecated' : 'tone-live'}`} onClick={() => navigate('/dashboard/invest/coin')}><span className="dashboard-tile-icon"><TrendingUp size={19}/></span><b className={momentumTrading ? (r >= 0 ? 'dashboard-positive' : 'dashboard-negative') : ''}>{momentumTrading ? `${r >= 0 ? '+' : ''}${r.toFixed(2)}%` : '-'}</b><span>{momentumTrading?.halted ? '⚠ 코인 정지됨' : '코인 (실거래·2x, 세션)'}</span></button> })()}
